...
- [x] 2단계 — 읽기 전용 도구를 진짜 원무브 데이터에 연결 (`02-real-data-readonly.js`): 실제
  ...
  **282명 전체 실행 결과(2차, 최종 — 2026-08-19)**: risk_level 반영 후 재실행. **총 103명 신고**
  (high 33 / medium 17 / low 53), 체험 미전환 179명 제외. 레뷰체험단 규칙도 정확히 적용됨(117일
  '레뷰연계'는 정식 포함, 14~20일짜리는 체험 제외). 애매한 케이스(예: 패스가 아직 시작 전인
  state 0 회원)도 근거를 들어 스스로 medium으로 분류하고 요약에 설명함. **2단계 검증 완료로
  판단** — risk_level별 요약이 실무적으로 바로 쓸 수 있는 수준.
- [ ] 3단계 — 메시지 초안 생성 도구 추가
